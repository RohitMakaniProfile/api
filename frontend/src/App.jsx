...
const BACKEND_URL = process.env.REACT_APP_BACKEND_URL || "http://136.112.16.53:8000";
...
function Navigation({ activeView, setActiveView, onNewChat }) {
  return (
    ...
    try {
      ...
      if (selectedImage) {
        ...
        if (!visionResponse.data.is_safe) {
          setMessages(prev => [...prev, {
            role: "luna",
            content: "Hey, I can't process this image. Let's share something else! 🌙",
            timestamp: new Date().toISOString()
          }]);
          setIsLoading(false);
    ...
    <div className="flex-1 flex flex-col max-w-5xl mx-auto w-full">
      ...
      >
        {messages.length === 0 && (
          <div className="text-center mt-16 animate-in fade-in slide-in-from-bottom duration-700">
            ...
            <p className="text-slate-400 mb-10 max-w-md mx-auto text-lg">
              Your intelligent companion who remembers everything. Share images, ask for photos, or just have a conversation!
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 max-w-3xl mx-auto">
              {quickSuggestions.map((suggestion, idx) => (
                ...
                >
                  <div className="text-3xl mb-2 group-hover:scale-110 transition-transform">{suggestion.icon}</div>
        ...
        {messages.map((msg, idx) => (
          ...
          >
            <div className={`w-10 h-10 rounded-2xl shrink-0 flex items-center justify-center text-lg shadow-xl ${
              msg.role === "user"
            ...
            <div className="max-w-[75%] lg:max-w-[65%] space-y-2">
                  ...
                  <div className="mb-3 rounded-xl overflow-hidden border border-white/10 shadow-lg">
                    <img
                      ...
                      onError={(e) => {
                        ...
                        e.target.src = "https://placehold.co/400x300/0f172a/10b981?text=Loading";
                      }}
                    />
                  </div>
                )}
              ...
              <p className={`text-xs text-slate-500 px-2 ${msg.role === "user" ? "text-right" : ""}`}>
  ...
  return (
    <div className="flex-1 px-4 py-6 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {images.map((img, idx) => (
            ...
            >
              ...
              <div className="p-4">
                <p className="text-sm text-slate-300 mb-2 line-clamp-2">{img.description}</p>
                <div className="flex flex-wrap gap-1.5">
                  {img.tags?.slice(0, 2).map((tag, tidx) => (
                    <span key={tidx} className="px-2 py-0.5 bg-emerald-500/20 text-emerald-400 rounded-full text-xs border border-emerald-500/30">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      {selectedImage && (
        ...
        >
          <div className="max-w-4xl w-full bg-slate-900 rounded-2xl overflow-hidden border border-white/20" onClick={(e) => e.stopPropagation()}>
            <img
            ...
            />
            <div className="p-6">
              <h3 className="text-xl font-semibold mb-2">{selectedImage.scene}</h3>
              <p className="text-slate-400 mb-4">{selectedImage.description}</p>
              <div className="flex flex-wrap gap-2">
                {selectedImage.tags?.map((tag, idx) => (
                  <span key={idx} className="px-3 py-1 bg-emerald-500/20 text-emerald-400 rounded-full text-sm border border-emerald-500/30">
                    {tag}
                  </span>
                ))}
              </div>
    ...
    <div className="flex-1 px-4 py-6 max-w-4xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-2xl p-6">
          <div className="text-3xl mb-2">👤</div>
          <div className="text-sm font-mono text-white">{userId.substring(0, 12)}...</div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-slate-500">
              All your data is stored locally on your device. Luna respects your privacy.
            </p>
          </div>
        </div>
